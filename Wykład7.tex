...
Kwadrat momentu pędu $\hat{L}^2$ we współrzędnych sferycznych wyraża się jako
$$
...
Przykładowe postacie funkcji $Y_l^m(\theta, \varphi)$
dla kilku małych parametrów $l$ i $m$ to
$$
...
Powyższe funkcje reprezentują różne przestrzenne rozkłady prawdopodobieństwa
--- zobrazowane na rysunkach jako chmury elektronowe (np. dla atomu wodoru).
\begin{figure}[H]
\centering
...
% To jest jakieś machanie nogami (nawet nie rękami). Nie ma absolutnie
% żadnego związku z definicją operatora J, która też nie jest specjalnie
% jasna.
...
\item wszystkie macierze $\hat{J}_x, \hat{J}_y, \hat{J}_z$ to macierze zerowe:
  ...
  % Taka już jest magia fizyki, że mówimy masę rzeczy bez sensu, zapominamy
  % o części i na końcu wychodzi coś.
  % FIXME !!
\end{itemize}
...
% Ho ho ho, spinory a spin to nie to samo. Wypadałoby powiedzieć coś
% na ten temat. Ja się nie znam, nie wiem co. FIXME
% Być może jest to jakieś niepoprawne użycie nazwy -- to też trzeba brać pod uwagę.
